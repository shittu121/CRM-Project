...
import Entryform from '@/components/company/vehicles/NewEntryForm';
import Editform from '@/components/company/vehicles/EditEntryForm';
import ViewVehicleform from '@/components/company/vehicles/ViewVehicle'
...
export default function VehiclePage() {
  ...
  const [isViewVehicleFormVisible, setIsViewVehicleFormVisible] = useState<boolean>(false);
  ...
  return (
    <div className="p-4">
      ...
      <h1 className="font-bold text-lg mb-10">Company Vehicles</h1>
      <div className="relative overflow-x-auto border border-stroke p-5  sm:rounded-lg">
        <div className="pb-5  flex flex-col sm:flex-row justify-between">
          <button
          ...
          <div className="relative mt-4 sm:mt-0 mx-2 w-full sm:w-auto">
            ...
            <input
              type="text"
              id="table-search"
              className="block text-black p-2 pl-10 text-sm border  outline-0 border-stroke hover:border-btnColor rounded-lg w-full sm:w-80"
          ...
          <tbody>
            {vehicles.map((vehicle, index) => (
              ...
              >
                ...
                <td
                  className={`pl-6 py-4 font-medium text-gray-900 whitespace-nowrap overflow-hidden text-ellipsis max-w-30 ${
                    vehicle.isDisabled ? "text-gray-500" : ""
                  }`}
                >
                  {vehicle.make}
                ...
                <td className="px-6 py-4 grid grid-cols-4 gap-4 lg:gap-2">
                  ...
                  <button
                    className="bg-red-600 hover:bg-red-700 py-2 px-4 rounded-md text-white text-base font-medium border border-red-600 transition hover:opacity-90 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed mr-3 col-span-2 lg:col-span-1"
                    disabled={vehicle.isDisabled}
                  >
                    Delete
                  </button>
                  <button
                    className="bg-green-600 hover:bg-green-700 py-2 px-4 rounded-md text-white text-base font-medium border border-green-600 transition hover:opacity-90 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed mr-3 col-span-2 lg:col-span-1"
                  ...
                  <button
                    className={`py-2 px-4 rounded-md text-white text-base font-medium border transition ${
                      vehicle.isDisabled
